    ...
    Private Sub mRptConvert()
        ...
        Try
            Using cnn As New SqlConnection(ConnectionString)
                cmd = New SqlCommand("select * from ActivatePolicy where CertificateNo='" & Trim(Session("mCert")) & "'", cnn)
                cnn.Open()
                rdr = cmd.ExecuteReader
                If rdr.Read Then
                    crpt = New ReportDocument
                    crpt.Load(Server.MapPath("Debitnote.rpt"))
                    ...
                    crpt.ExportToHttpResponse(ExportFormatType.PortableDocFormat, Response, False, "Debitnote.rpt")
                    crpt.Refresh()
                End If
                cnn.Close()
            End Using
